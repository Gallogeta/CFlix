document.addEventListener("DOMContentLoaded", () => {
    ...
    const btnCancelSingleUpload = document.getElementById("btn-cancel-single-upload");
    const btnCancelMassIngest = document.getElementById("btn-cancel-mass-ingest");
    let activeChunkUploadCancel = null;
    ...
    const selectMonitoredFolder = document.getElementById("select-monitored-folder");
    const btnCustomWatchFolder = document.getElementById("btn-custom-watch-folder");
    const monitoredFolderStatusTag = document.getElementById("monitored-folder-status-tag");
    const btnCleanPlaceholders = document.getElementById("btn-clean-placeholders");
    ...
    tabButtons.forEach(btn => {
        btn.addEventListener("click", () => {
            ...
            if (targetId === "queue-tab") { loadMonitoredFolders(); loadIncomingQueue(); }
    ...
    function renderDynamicLibraryCards() {
        ...
        mediaDirectories.forEach((d, idx) => {
            ...
            const driveCount = (d.locations && d.locations.filter(l => l.exists).length) || 1;
            const storageTag = d.multi_drive ? `⚡ Multi-Drive (${driveCount})` : (d.locations && d.locations[0] ? d.locations[0].disk_name : "Storage");
    ...
    function renderDirectoriesTable() {
        ...
        mediaDirectories.forEach(d => {
            ...
            const driveCount = (d.locations && d.locations.filter(l => l.exists).length) || 1;
            const driveLabel = driveCount > 1 ? `⚡ Multi-Drive (${driveCount} Drives)` : "Single Drive";
            const storageBadge = d.multi_drive ?
                `<div style="display:flex;flex-direction:column;gap:2px;">
                    <span class="pill pill-blue" style="font-size:0.7rem;width:fit-content;">${driveLabel}</span>
                    <code style="font-size:0.72rem;">${d.locations.filter(l => l.exists).map(l => l.path).join(' + ')}</code>
                </div>` :
                `<code>${d.path}</code>`;
    ...
    async function startMassIngest() {
        ...
        if (btnCancelMassIngest) {
            btnCancelMassIngest.disabled = false;
            btnCancelMassIngest.textContent = "🛑 Stop Ingestion";
            btnCancelMassIngest.onclick = async () => {
                if (!isBatchProcessing) return;
                if (confirm("Stop mass ingestion? Completed episodes will remain safely in your library.")) {
                    isBatchCancelled = true;
                    btnCancelMassIngest.disabled = true;
                    btnCancelMassIngest.textContent = "Stopping...";
                    if (activeChunkUploadCancel) {
                        await activeChunkUploadCancel();
                    }
                }
            };
        }
        ...
        for (let i = 0; i < totalItems; i++) {
            if (isBatchCancelled) {
                console.log("Mass ingest loop cancelled by user.");
                break;
            }
            ...
            if (!item.is_server_file && item.file) {
                ...
                try {
                    ...
                    if (isBatchCancelled || (err.message && err.message.includes("cancelled"))) {
                        item.status = "cancelled";
                        if (statusCell) statusCell.innerHTML = `<span class="stat-tag" style="background: rgba(251,191,36,0.2); color:#fde047;">Cancelled</span>`;
                        break;
                    }
                    item.status = "error";
        ...
        if (isBatchCancelled) {
            batchProgressTitle.textContent = `Batch Cancelled (${completedCount} completed, ${totalItems - completedCount} stopped)`;
            batchCurrentFileText.textContent = `Ingestion stopped safely. Completed episodes remain saved. Server is idle for shutdown.`;
            ...
            alert(`🛑 Batch ingestion was stopped!\n${completedCount} episodes were successfully ingested.\nThe remaining ${totalItems - completedCount} episodes were cancelled so your PC can shut down safely.`);
        } else {
    ...
    btnRefreshQueue.addEventListener("click", () => { loadMonitoredFolders(); loadIncomingQueue(); });

    async function loadMonitoredFolders() {
        if (!selectMonitoredFolder) return;
        try {
            const resp = await apiFetch("/api/watcher/folders");
            const data = await resp.json();
            const current = data.current || "";
            selectMonitoredFolder.innerHTML = "";
            (data.folders || []).forEach(f => {
                const opt = document.createElement("option");
                opt.value = f;
                opt.textContent = f === current ? `${f} (Active)` : f;
                if (f === current) opt.selected = true;
                selectMonitoredFolder.appendChild(opt);
            });
            if (monitoredFolderStatusTag) {
                monitoredFolderStatusTag.textContent = data.enabled ? "● Watching" : "○ Paused";
                monitoredFolderStatusTag.className = data.enabled ? "folder-status-tag" : "folder-status-tag inactive";
            }
        } catch (e) {
            console.error("Error loading monitored folders:", e);
        }
    }

    if (selectMonitoredFolder) {
        selectMonitoredFolder.addEventListener("change", async () => {
            const folder = selectMonitoredFolder.value;
            if (!folder) return;
            try {
                const resp = await apiFetch("/api/watcher/set-folder", {
                    method: "POST",
                ...
                if (res.success) {
                    ...
                    await loadIncomingQueue();
                } else {
                    alert(`Failed to switch folder: ${res.error}`);
                }
            } catch (e) {
                alert(`Error switching monitored folder: ${e.message}`);
            }
        });
    }

    if (btnCustomWatchFolder) {
        btnCustomWatchFolder.addEventListener("click", async () => {
            const current = selectMonitoredFolder ? selectMonitoredFolder.value : "/home/gallo/dwhelper";
            const customPath = prompt("Enter full directory path on server to monitor:", current);
            if (!customPath || !customPath.trim()) return;
            try {
                const resp = await apiFetch("/api/watcher/set-folder", {
                    method: "POST",
                    body: JSON.stringify({ folder: customPath.trim(), create_if_missing: true })
                });
                const res = await resp.json();
                if (res.success) {
                    await loadMonitoredFolders();
                    await loadIncomingQueue();
                    alert(`Monitored folder set to: ${res.watch_dir} (${res.file_count} media files found)`);
                } else {
                    alert(`Failed to set folder: ${res.error}`);
                }
            } catch (e) {
                alert(`Error: ${e.message}`);
            }
    ...
    if (btnCleanPlaceholders) {
        btnCleanPlaceholders.addEventListener("click", async () => {
            if (!confirm("Permanently clean redundant .disc placeholders on disk for movies you already own?")) return;
            btnCleanPlaceholders.disabled = true;
            btnCleanPlaceholders.innerHTML = "<span>Cleaning Placeholders...</span>";
            try {
                const resp = await apiFetch("/api/collections/clean-placeholders", { method: "POST" });
                const res = await resp.json();
                alert(`Cleaned ${res.purged_count} redundant placeholder(s)!`);
                loadCollections();
            } catch (e) {
                alert(`Error cleaning placeholders: ${e.message}`);
            } finally {
                btnCleanPlaceholders.disabled = false;
                btnCleanPlaceholders.innerHTML = `
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
                    <span>Clean Placeholders</span>
                `;
            }
        });
    }
    ...
    function renderCollections(colls) {
        ...
        colls.forEach(c => {
            ...
            card.innerHTML = `
                <div class="collection-header">
                    ...
                    <div class="collection-stats">
                        ...
                        <button class="btn-sync-card" data-cid="${c.id}" data-cname="${escapeHtml(c.name)}" title="Sync ${escapeHtml(c.name)}">
            ...
            `;
            const cardSyncBtn = card.querySelector(".btn-sync-card");
            if (cardSyncBtn) {
                cardSyncBtn.addEventListener("click", async (e) => {
                    e.stopPropagation();
                    cardSyncBtn.disabled = true;
                    cardSyncBtn.innerHTML = "<span>...</span>";
                    try {
                        const resp = await apiFetch("/api/collections/sync-one", {
                            method: "POST",
    ...
    async function checkPath(path, badgeEl, autoCreate = false, category = null, chipsContainerId = null) {
        if (!path || path.trim() === "") {
            if (badgeEl) {
            ...
            if (chipsContainerId) {
                const cEl = document.getElementById(chipsContainerId);
        ...
        try {
            const resp = await apiFetch("/api/settings/validate-path", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ path, category, create_if_missing: autoCreate })
            });
            const res = await resp.json();

            if (res.exists && res.is_dir) {
                const writableBadge = res.writable ? "" : " (Read-Only)";
                if (res.multi_drive && res.locations && res.locations.length > 1) {
                    const activeT = res.active_target;
                    const activeFree = activeT ? activeT.free_human : res.free_human;
                    const activeDisk = activeT ? activeT.disk_name : res.disk_name;
                    badgeEl.className = res.writable ? "path-badge valid" : "path-badge missing";
                    badgeEl.innerHTML = `⚡ ${res.locations.length} Drives (${activeFree} Available • Target: ${activeDisk}${writableBadge})`;
                } else {
            ...
            // Render multi-drive location chips
            if (chipsContainerId) {
                const cEl = document.getElementById(chipsContainerId);
                if (cEl) {
                    if (res.locations && res.locations.length > 1) {
                        const activePath = res.active_target ? res.active_target.path : "";
                        cEl.innerHTML = res.locations.map((loc, idx) => {
                            const isAct = loc.path === activePath;
                            const isFull = loc.free_gb < 50;
                            const statusHtml = isAct
                                ? `<span class="chip-status active">⚡ Write Target</span>`
                                : (isFull ? `<span class="chip-status full">Full (${loc.free_human})</span>` : `<span class="chip-status">${loc.free_human} Free</span>`);
                            const prefix = idx === 0 ? "Primary" : "Expansion";
                            return `
                                <div class="lib-drive-chip ${isAct ? 'active' : ''}" title="${escapeHtml(loc.path)} (${prefix} Drive)">
                                    <span class="chip-name">💾 ${escapeHtml(loc.disk_name)}:</span>
                                    <code style="font-size:0.68rem;">${escapeHtml(loc.path)}</code>
                                    ${statusHtml}
                                </div>
                            `;
                        }).join("");
    ...
    function validateAllPaths(autoCreate = false) {
        checkPath(inputMoviesDir.value, badgeMoviesDir, autoCreate, "movies", "chips-movies-dir");
        checkPath(inputSeriesDir.value, badgeSeriesDir, autoCreate, "series", "chips-series-dir");
        if (inputAnimeDir && badgeAnimeDir) checkPath(inputAnimeDir.value, badgeAnimeDir, autoCreate, "anime", "chips-anime-dir");
        checkPath(inputAdultDir.value, badgeAdultDir, autoCreate, "adult", "chips-adult-dir");
        checkPath(inputWatchDir.value, badgeWatchDir, autoCreate, null, null);
    }
    ...
    // ==========================================
    // MULTI-SERVER MANAGEMENT & SWITCHING ENGINE
    // ==========================================
    let serversData = [];
    let activeServerId = "server-local";

    const btnServerSwitcher = document.getElementById("btn-server-switcher");
    const serverDropdownMenu = document.getElementById("server-dropdown-menu");
    const serverDropdownList = document.getElementById("server-dropdown-list");
    const switcherActiveServerName = document.getElementById("switcher-active-server-name");
    const uploadTargetServerSelect = document.getElementById("upload-target-server-select");
    const settingsServersList = document.getElementById("settings-servers-list");
    const btnOpenAddServer = document.getElementById("btn-open-add-server");
    const btnSettingsAddServer = document.getElementById("btn-settings-add-server");

    const serverModal = document.getElementById("server-modal");
    const serverModalTitle = document.getElementById("server-modal-title");
    const serverModalId = document.getElementById("server-modal-id");
    const serverModalName = document.getElementById("server-modal-name");
    const serverModalUrl = document.getElementById("server-modal-url");
    const serverModalUser = document.getElementById("server-modal-user");
    const serverModalPass = document.getElementById("server-modal-pass");
    const serverModalMovies = document.getElementById("server-modal-movies");
    const serverModalSeries = document.getElementById("server-modal-series");
    const serverModalRoots = document.getElementById("server-modal-roots");
    const serverModalWatch = document.getElementById("server-modal-watch");
    const serverModalSetActive = document.getElementById("server-modal-set-active");
    const btnModalTestServer = document.getElementById("btn-modal-test-server");
    const serverModalTestResult = document.getElementById("server-modal-test-result");
    ...
    async function loadServers() {
        try {
            ...
            activeServerId = data.active_id || "server-local";

            const activeSrv = serversData.find(s => s.id === activeServerId) || serversData[0];
            if (activeSrv && switcherActiveServerName) {
                switcherActiveServerName.textContent = activeSrv.name || "Media Server";
            }

            // 1. Populate Header Dropdown
            if (serverDropdownList) {
                serverDropdownList.innerHTML = "";
                serversData.forEach(s => {
                    const item = document.createElement("div");
                    const isActive = s.id === activeServerId;
                    item.className = "server-dropdown-item" + (isActive ? " active" : "");
                    item.innerHTML = `
                        <div class="server-item-info">
                            <span class="server-item-name">${escapeHtml(s.name)}</span>
                            <span class="server-item-url">${escapeHtml(s.url)}</span>
                        </div>
                        <div style="display: flex; align-items: center; gap: 6px;">
                            ${isActive ? '<span class="server-item-badge">Active</span>' : '<button type="button" class="btn-secondary btn-xs btn-quick-switch" style="padding: 2px 8px; font-size: 0.72rem; cursor: pointer;">Switch</button>'}
                        </div>
                    `;
                    item.addEventListener("click", (e) => {
                        if (serverDropdownMenu) serverDropdownMenu.classList.add("hidden");
                        if (!isActive) switchServer(s.id);
                    });
                    serverDropdownList.appendChild(item);
                });
            }

            // 2. Populate Upload Target Dropdown (if present)
            if (uploadTargetServerSelect) {
                uploadTargetServerSelect.innerHTML = "";
                serversData.forEach(s => {
                    const opt = document.createElement("option");
                    opt.value = s.id;
                    const hostShort = s.url.replace("http://", "").replace("https://", "").split("/")[0];
                    opt.textContent = `${s.name} (${hostShort})`;
                    if (s.id === activeServerId) opt.selected = true;
                    uploadTargetServerSelect.appendChild(opt);
                });
            }

            // 3. Populate Settings Tab Cards
            if (settingsServersList) {
                settingsServersList.innerHTML = "";
                serversData.forEach(s => {
                    const isActive = s.id === activeServerId;
                    const card = document.createElement("div");
                    card.className = "server-manager-card" + (isActive ? " active" : "");
                    card.innerHTML = `
                        <div class="server-manager-details">
                            <h4>
                                <span>${escapeHtml(s.name)}</span>
                                ${isActive ? '<span class="pill pill-green">ACTIVE</span>' : ""}
                            </h4>
                            <div class="server-manager-url">${escapeHtml(s.url)} (${escapeHtml(s.user || "no user")})</div>
                            <div class="server-manager-paths">
                                Movies: <code>${escapeHtml(s.movies_dir || "/media/movies")}</code> | Series: <code>${escapeHtml(s.series_dir || "/media/tv")}</code>
                            </div>
                        </div>
                        <div class="server-manager-actions">
                            ${!isActive ? `<button class="btn-primary btn-sm btn-activate-srv" data-id="${s.id}">Set Active</button>` : ""}
                            <button class="btn-secondary btn-sm btn-edit-srv" data-id="${s.id}">Edit</button>
                            ${serversData.length > 1 ? `<button class="btn-secondary btn-sm btn-danger-text btn-del-srv" data-id="${s.id}">Delete</button>` : ""}
                        </div>
                    `;

                    const btnAct = card.querySelector(".btn-activate-srv");
                    if (btnAct) btnAct.addEventListener("click", () => switchServer(s.id));

                    const btnEdit = card.querySelector(".btn-edit-srv");
                    if (btnEdit) btnEdit.addEventListener("click", () => openServerModal(s));

                    const btnDel = card.querySelector(".btn-del-srv");
                    if (btnDel) btnDel.addEventListener("click", () => deleteServer(s.id, s.name));

                    settingsServersList.appendChild(card);
                });
            }
        } catch (e) {
    ...
    async function switchServer(serverId) {
        try {
            const resp = await apiFetch("/api/servers/switch", {
                method: "POST",
            ...
            if (res.success) {
                ...
                if (activeTab === "collections-tab" && typeof loadCollections === "function") loadCollections();
                if (activeTab === "queue-tab") {
                    if (typeof loadMonitoredFolders === "function") loadMonitoredFolders();
                    if (typeof loadIncomingQueue === "function") loadIncomingQueue();
                }
                alert(`Successfully switched active media server to '${res.server.name}'!`);
            } else {
                alert(`Failed to switch server: ${res.error}`);
            }
        } catch (e) {
            alert(`Error switching server: ${e.message}`);
        }
    }

    if (btnServerSwitcher) {
        btnServerSwitcher.addEventListener("click", (e) => {
            e.stopPropagation();
            if (serverDropdownMenu) serverDropdownMenu.classList.toggle("hidden");
        });
        document.addEventListener("click", (e) => {
            if (serverDropdownMenu && !serverDropdownMenu.contains(e.target) && !btnServerSwitcher.contains(e.target)) {
    ...
    if (uploadTargetServerSelect) {
        uploadTargetServerSelect.addEventListener("change", () => {
            const targetId = uploadTargetServerSelect.value;
            if (targetId && targetId !== activeServerId) {
                switchServer(targetId);
            }
        });
    }
    ...
    function openServerModal(srv = null) {
        ...
        if (serverModalTestResult) {
            serverModalTestResult.textContent = "Not tested";
            serverModalTestResult.className = "conn-status-tag";
        }

        if (srv) {
            if (serverModalTitle) serverModalTitle.textContent = "Edit Media Server";
            if (serverModalId) serverModalId.value = srv.id || "";
            if (serverModalName) serverModalName.value = srv.name || "";
            if (serverModalUrl) serverModalUrl.value = srv.url || "";
            if (serverModalUser) serverModalUser.value = srv.user || "";
            if (serverModalPass) {
                serverModalPass.value = "";
                serverModalPass.placeholder = srv.has_pass ? "•••••••• (Unchanged)" : "Password";
            }
            if (serverModalMovies) serverModalMovies.value = srv.movies_dir || "";
            if (serverModalSeries) serverModalSeries.value = srv.series_dir || "";
            if (serverModalRoots) serverModalRoots.value = Array.isArray(srv.media_roots) ? srv.media_roots.join(", ") : (srv.media_roots || "");
            if (serverModalWatch) serverModalWatch.value = srv.watch_dir || "";
            if (serverModalSetActive) serverModalSetActive.checked = (srv.id === activeServerId);
        } else {
            if (serverModalTitle) serverModalTitle.textContent = "Add Media Server";
            if (serverModalId) serverModalId.value = "";
            if (serverModalName) serverModalName.value = "";
            if (serverModalUrl) serverModalUrl.value = "http://";
            if (serverModalUser) serverModalUser.value = "";
            if (serverModalPass) {
                serverModalPass.value = "";
                serverModalPass.placeholder = "Password";
            }
            if (serverModalMovies) serverModalMovies.value = "/media/movies";
            if (serverModalSeries) serverModalSeries.value = "/media/tv";
            if (serverModalRoots) serverModalRoots.value = "/media";
            if (serverModalWatch) serverModalWatch.value = "/downloads";
            if (serverModalSetActive) serverModalSetActive.checked = true;
        }
    ...
    if (btnCancelServerModal) btnCancelServerModal.addEventListener("click", closeServerModal);
    if (serverModal) {
    ...
    document.addEventListener("keydown", (e) => {
        if (e.key === "Escape" && serverModal && !serverModal.classList.contains("hidden")) {
            closeServerModal();
        }
    });

    if (btnOpenAddServer) {
        btnOpenAddServer.addEventListener("click", (e) => {
            e.preventDefault();
            e.stopPropagation();
            if (serverDropdownMenu) serverDropdownMenu.classList.add("hidden");
    ...
    if (btnSettingsAddServer) {
        btnSettingsAddServer.addEventListener("click", (e) => {
            e.preventDefault();
            e.stopPropagation();
            openServerModal(null);
        });
    ...
    if (btnModalTestServer) {
        btnModalTestServer.addEventListener("click", async () => {
            const url = serverModalUrl ? serverModalUrl.value.trim() : "";
            const user = serverModalUser ? serverModalUser.value.trim() : "";
            const pass = serverModalPass ? serverModalPass.value : "";
            const serverId = serverModalId ? serverModalId.value.trim() : "";
            ...
            btnModalTestServer.disabled = true;
            if (serverModalTestResult) {
                serverModalTestResult.textContent = "Testing handshake...";
                serverModalTestResult.className = "conn-status-tag testing";
            }

            try {
                const resp = await apiFetch("/api/servers/test", {
                    method: "POST",
                    body: JSON.stringify({ url, user, pass, server_id: serverId })
                });
                const res = await resp.json();
                if (res.success) {
                    if (serverModalTestResult) {
                        serverModalTestResult.textContent = res.message || "Connected!";
                        serverModalTestResult.className = "conn-status-tag success";
                    }
                } else {
                    if (serverModalTestResult) {
                        serverModalTestResult.textContent = res.error || "Failed";
                        serverModalTestResult.className = "conn-status-tag failed";
                    }
                }
            } catch (e) {
                if (serverModalTestResult) {
                    serverModalTestResult.textContent = "Error: " + e.message;
                    serverModalTestResult.className = "conn-status-tag failed";
                }
            } finally {
                btnModalTestServer.disabled = false;
            }
        });
    }

    if (btnSaveServerModal) {
        btnSaveServerModal.addEventListener("click", async () => {
            const name = serverModalName ? serverModalName.value.trim() : "";
            const url = serverModalUrl ? serverModalUrl.value.trim() : "";
            if (!name || !url) {
            ...
            const shouldSetActive = serverModalSetActive ? serverModalSetActive.checked : false;

            const payload = {
                id: serverModalId ? serverModalId.value.trim() || undefined : undefined,
                name: name,
                url: url,
                user: serverModalUser ? serverModalUser.value.trim() : "",
                pass: serverModalPass ? serverModalPass.value : "",
                movies_dir: serverModalMovies ? serverModalMovies.value.trim() : "",
                series_dir: serverModalSeries ? serverModalSeries.value.trim() : "",
                media_roots: serverModalRoots ? serverModalRoots.value.trim() : "",
                watch_dir: serverModalWatch ? serverModalWatch.value.trim() : ""
            };

            btnSaveServerModal.disabled = true;
            btnSaveServerModal.textContent = "Saving...";
            try {
                const resp = await apiFetch("/api/servers/save", {
                    method: "POST",
                ...
                if (res.success) {
                    closeServerModal();
                    await loadServers();
                    if (shouldSetActive && res.server && res.server.id) {
                        await switchServer(res.server.id);
                    } else {
                        alert(`Saved server '${res.server.name}' successfully!`);
                    }
                } else {
                ...
                btnSaveServerModal.disabled = false;
                btnSaveServerModal.textContent = "Save Server";
            }
        });
    }
    ...
    async function deleteServer(serverId, serverName) {
        ...
        try {
            const resp = await apiFetch("/api/servers/delete", {
                method: "POST",
                body: JSON.stringify({ server_id: serverId })
            });
            const res = await resp.json();
            if (res.success) {
                ...
                loadSettings();
                loadMediaDirectories();
                alert(`Deleted server '${serverName}'.`);
            } else {
